...
@app.route('/api/scrum/fix', methods=['POST'])
@token_required
def apply_fixes(current_user):
    """Apply chosen fixes to a cahier de charge and return an enhanced spec"""
    try:
        data = request.get_json() or {}

        fixes = data.get('fixes') or data.get('chosen_fixes') or []
        print("Received fixes:", fixes)
        original = data.get('originalDocument') or data.get('original_document') or data.get('cahier_de_charge') or ''
        print("Received original document:", original[:200], "...")
        manual_patch = data.get('manual_patch', '')

        if not original:
            return jsonify({"error": "Missing original document"}), 400

        # Normalize fixes: ensure list of dicts
        if not isinstance(fixes, list):
            return jsonify({"error": "fixes must be a list"}), 400

        # Prepare payload for merge_app
        payload = {
        ...
        # Call the merge graph from scrum_graph
        if not hasattr(scrum_graph, 'merge_app'):
            return jsonify({"error": "merge_app not available on scrum_graph"}), 500

        try:
            result = scrum_graph.merge_app.invoke(payload)
            print("Merge result:", result)
        except Exception as e:
            # If the LLM or merge fails, return error
            return jsonify({"error": "Failed to merge fixes", "detail": str(e)}), 500

        spec_enhanced = result.get('spec_enhanced')
        merge_report = result.get('merge_report') or {}

        return jsonify({
            "success": True,
            "spec_enhanced": spec_enhanced,
